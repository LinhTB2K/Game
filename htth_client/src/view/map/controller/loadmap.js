let loadMap = (self) => {
        ...
        if(e.length >= 1) {
            ...
            if(1+1===2) {
                ...
                if(checked.length <=1) {
                    ...
                    if(!find) {
                        if(gameMap.setting.maxY < element.y) gameMap.setting.maxY = element.y;
                        if(gameMap.setting.minY > element.y - element.height) gameMap.setting.minY = element.y - element.height;
                        if(gameMap.setting.maxX < element.x + element.width) gameMap.setting.maxX = element.x + element.width;
                        if(gameMap.setting.minX > element.x) gameMap.setting.minX = element.x;
                        continue;
                    }
                ...
                {
                    const textures = checked.map(e => {
                        return PIXI.Texture.from('./assets/map/dat/'+e+'.png')
                    });
    ...
    // Load custom Background Image from res_map.js' bg property automatically
    let mapid = self.my.pos.map;
    let mapData = self.listMap ? self.listMap.find(e => e.id === mapid) : null;
    if (mapData && mapData.bg) {
        let isWorldIsland = ['Starter_Island', 'Jungle_Island', 'Desert Island', 'Marine_Base', 'Sky_Island'].includes(mapData.bg);
        let customBgSrc = isWorldIsland ? './src/view/image/' + mapData.bg + '.png' : './src/view/image/' + mapData.bg + '.png'; 
        
        let existingMainBg = background.find(e => e.name == 'custom_map_bg');
        if(!existingMainBg) {
            background.push({
                name: 'custom_map_bg',
                type: 'nui1',
                ...
                x: 0,
                y: 0,
                width: 1500, // Safe broad width cover for the image
                height: 700  // Standard height boundary
            });
        }
    }
