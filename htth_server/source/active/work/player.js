...
let congberi = (socket, value) => {
    let my = socket.my;
    my = int04.info(my);
    my.tien.beri += value;
    int04.setPlayer(my);
    // Refresh client money
    socket.send([my.tien], -10);
};

let add_item = (socket, data) => {
    let my = socket.my;
    // Basic Add Item to inventory implementation
    let itemObj = int04.addItem(data.name, data.quantity);
    if(itemObj) {
        my.ruong.data.push(itemObj);
        int04.setPlayer(my);
        // Refresh inventory loosely
        socket.send([my.ruong], -11);

        // Quest Collect Trigger
        let questController = require('../../int04/quest/questController.js');
        questController.updateProgress(socket, 'collect', data.name, data.quantity);
    }
};
...
module.exports = function(io,data) {
    ...
    if(data.action == 'congberi') return congberi(socket, data.value);
    if(data.action == 'add_item') return add_item(socket, data);
