...
module.exports = async function(socket,data) {
    ...
    // Load quests into memory
    let questController = require('../quest/questController.js');
    await questController.initPlayer(socket);

    socket.send(my,-3);

    // Send the loaded quests independently so Client can render the tracker
    questController.sendUpdateToClient(socket);
